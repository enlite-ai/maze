...
# Set up Maze installation (all dependencies should already have been installed via maze-core-environment.yml).
WORKDIR /usr/src/maze
COPY setup.py setup.py
COPY maze/__init__.py maze/__init__.py
...
####################################################
# Complete maze-core image.
####################################################

# Assemble maze-core from built environment and code.
FROM maze_core_env as maze_core
WORKDIR /usr/src/
COPY --from=maze_core_code /usr/src/maze maze/