""" Implements observation stacking by actor wrapper. Adapted from ObservationStackWrapper"""
import copy
...
class ObservationStackByActorIDWrapper(ObservationWrapper[MazeEnv]):
    ...
    def __init__(self, env: StructuredEnvSpacesMixin, stack_config: List[Dict[str, Any]]):
        ...
        self._observation_stack: Dict[ActorID, Dict[str, List[np.ndarray]]] = defaultdict(lambda: defaultdict(list))
    ...
    def observation(self, observation: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        for config in self.stack_config:
            ...
            if obs_key in observation:
                ...
                self._observation_stack[actor_id][obs_key] = self._observation_stack[actor_id][obs_key][-self.max_steps:]
    ...
    def clear_actor_data(self, actor_id: ActorID):
        """Clear the actor data."""
        self._observation_stack[actor_id] = defaultdict(list)

    @override(ObservationWrapper)
    def reset(self) -> Dict[str, np.ndarray]:
        """Intercept ``ObservationWrapper.reset`` and map observation."""
        # reset observation stack
        self.clear_actor_data(self.actor_id())
        return super().reset()
    ...
    def set_observation_stack(self, observation_stack: Dict[ActorID, Dict[str, List[np.ndarray]]]) -> None:
    ...
    def get_observation_stack(self) -> Dict[ActorID, Dict[str, List[np.ndarray]]]:
