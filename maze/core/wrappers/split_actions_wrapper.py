...
class SplitActionsWrapper(ActionWrapper[EnvType | StructuredEnvSpacesMixin | StructuredEnv]):
    ...
    def __init__(
        ...
        split_config: dict[str, dict[str, dict[str, tuple | list]]],
    ):
        super().__init__(env)
        assert isinstance(self.env.action_space, gym.spaces.Dict)
        self._test_config(split_config)
        self.split_config = split_config
        self.reverse_action_ref = {
            sub_action_name: org_action_name
        ...
        self._action_spaces_dict = {
            step_key: self._split_action_space(action_space)
            for step_key, action_space in self.env.action_spaces_dict.items()
        }

    def _test_config(self, split_config: dict[str, dict[str, dict[str, tuple | list]]]) -> None:
        ...
        for old_action_name, action_split_config in split_config.items():
            if isinstance(
                self.env.action_space.spaces[old_action_name],
                ...
                all_indices = [
                    item for sub_action_config in action_split_config.values() for item in sub_action_config['indices']
                ]
                assert org_action_space.shape[-1] == len(set(all_indices))
            else:
                raise NotImplementedError(
                    'The split action wrapper is currently only implemented for box, '
            ...
            if org_action_name in self.split_config:
                for new_action_key, new_space_config in self.split_config[org_action_name].items():
                    if isinstance(org_action_space, gym.spaces.Box):
                        ...
                        new_action_space[new_action_key] = gym.spaces.Box(
                            low=np.asarray(low),
        ...
        for action_name, action_value in action.items():
            if action_name in self.reverse_action_ref:
                ...
                if self.reverse_action_ref[action_name] not in tmp_corresponding_actions:
                    tmp_corresponding_actions[self.reverse_action_ref[action_name]] = {}
        ...
        for org_action_name, org_action in action.items():
            if org_action_name in self.split_config:
                for new_action_key, new_action_config in self.split_config[org_action_name].items():
                    new_action[new_action_key] = np.stack(
                        [org_action[select_index] for select_index in new_action_config['indices']]
                    )
            else:
