...
from maze.core.log_events.log_create_figure_functions import (
    create_binary_plot,
...
class ActionEvents(ABC):
    ...
    def multi_binary_action(
        self,
        step_key: str,
        agent_name: str,
        name: str,
        value: int,
        num_binary_actions: int,
    ):
        """action taken"""
...
class RewardEvents(ABC):
    ...
    @define_epoch_stats(np.min, input_name='sum', output_name='min')
    @define_epoch_stats(np.mean, input_name='sum', output_name='mean')
    @define_epoch_stats(np.mean, input_name='episode_mean', output_name='step_mean')
    @define_epoch_stats(np.std, input_name='sum', output_name='std')
    ...
    @define_episode_stats(np.mean, output_name='episode_mean')
    @define_episode_stats(np.sum, output_name='sum')
