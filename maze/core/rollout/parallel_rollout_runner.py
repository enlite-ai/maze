...
class ParallelRolloutWorker:
    ...
    def _setup_monitoring(
        env: StructuredEnv, record_trajectory: bool, serialize_renderer: bool
    ) -> tuple[StructuredEnv, EpisodeRecorder]:
...
class ParallelRolloutRunner(RolloutRunner):
    ...
    def _launch_workers(
        ...
        for env_seed, agent_seed in zip(env_seeds, agent_seeds, strict=False):
        ...
        workers = self._configure_and_launch_processes(
            parallel_worker_type=ParallelRolloutWorker,
            env=env,
            wrappers=wrappers,
            agent=agent,
        )
    ...
    def _configure_and_launch_processes(
        self,
        parallel_worker_type: type(ParallelRolloutWorker),
        env: ConfigType,
        wrappers: CollectionOfConfigType,
        agent: ConfigType,
    ) -> Iterable[Process]:
        """Configure and launch the processes."""
        ...
        for _ in tqdm(range(self.n_episodes), desc='Episodes done', unit=' episodes'):
            ...
            if isinstance(report, ExceptionReport):
                for p in workers:
                    p.terminate()
                raise RuntimeError(
                    f'A worker encountered the following error on env_seed: {report.env_seed} with '
