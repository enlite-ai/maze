...
class TorchActorCritic(TorchModel):
    ...
    def __init__(
        self,
        policy: TorchPolicy,
        critic: TorchStateCritic | TorchStateActionCritic,
        device: str,
    ):
        # check if appropriate models are provided
        assert critic is not None and isinstance(critic, (TorchStateCritic, TorchStateActionCritic)), (
    ...
    def compute_actor_critic_output(
        self, record: StructuredSpacesRecord, temperature: float = 1.0
