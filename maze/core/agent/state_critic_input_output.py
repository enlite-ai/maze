...
class StateCriticOutput:
    ...
    def __init__(self):
        self._step_critic_outputs: list[StateCriticStepOutput] = []
...
class StateCriticStepInput:
    """State Critic input for a single substep of the env, holding the tensor_dict and the actor_ids corresponding to
    where the embedding logits where retrieved if applicable, otherwise just the corresponding actor.
    ...
    def build(cls, policy_step_output: PolicySubStepOutput, observation: TorchObservationType) -> StateCriticStepInput:
        ...
        if policy_step_output.embedding_logits is not None:
            ...
            assert len(set(combined_keys)) == len(combined_keys), (
                'Duplicates in critic input found, please make sure that no shared embedding keys/outputs have the '
                'same name as any observation.'
            )
