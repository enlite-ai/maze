...
def test_beta_sample():
    ...
    dist = BetaProbabilityDistribution(
        logits=logits, action_space=Box(low=0.0, high=10.0, shape=(3,), dtype=np.float64), temperature=1.0
    )
...
def test_beta_entropy():
    ...
    logits = torch.from_numpy(np.random.randn(100, 6))
    dist = BetaProbabilityDistribution(
        logits=logits, action_space=Box(low=0.0, high=10.0, shape=(100, 3), dtype=np.float64), temperature=1.0
    )
...
def test_beta_kl():
    ...
    dist_0 = BetaProbabilityDistribution(
        logits=logits_0, action_space=Box(low=0.0, high=10.0, shape=(3,), dtype=np.float64), temperature=1.0
    )
    dist_1 = BetaProbabilityDistribution(
        logits=logits_1, action_space=Box(low=0.0, high=10.0, shape=(3,), dtype=np.float64), temperature=1.0
    )
