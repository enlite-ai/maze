...
def test_concat_block():
    ...
    for d0, d1 in [([100, 16], [100, 16]), ([100, 1, 16], [100, 1, 16])]:
        ...
        net = ConcatenationBlock(
            in_keys=['in_key_0', 'in_key_1'], out_keys='concat', in_shapes=[d0[1:], d1[1:]], concat_dim=-1
        )
        str(net)
...
def test_mlp_and_concat():
    ...
    net = ConcatenationBlock(in_keys=list(feat_dict.keys()), out_keys='concat', in_shapes=[(32,), (32,)], concat_dim=-1)
...
def test_correlation_block():
    ...
    for d0, d1 in [
        ...
        # with out reduction
        net = CorrelationBlock(
            in_keys=['in_key_0', 'in_key_1'], out_keys='correlation', in_shapes=[d0[1:], d1[1:]], reduce=False
        )
        str(net)
        ...
        # with reduction
        net = CorrelationBlock(
            in_keys=['in_key_0', 'in_key_1'], out_keys='correlation', in_shapes=[d0[1:], d1[1:]], reduce=True
        )
        str(net)
...
def test_action_masking_block():
    ...
    # with out reduction
    net = ActionMaskingBlock(
        in_keys=['logits', 'mask'],
        out_keys='masked',
        in_shapes=[logits.shape[1:], mask.shape[1:]],
        num_actors=1,
