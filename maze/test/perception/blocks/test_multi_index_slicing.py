...
def test_multi_index_slicing_block_multiple_ids():
    ...
    select_block = MultiIndexSlicingBlock(
        in_keys='mask', out_keys='selected', in_shapes=in_shape, select_dim=-1, select_idxs=[0, 2]
    )
...
def test_multi_index_slicing_block_single():
    ...
    in_shape = (mask.shape[-1],)
    select_block = MultiIndexSlicingBlock(
        in_keys='mask', out_keys='selected', in_shapes=in_shape, select_dim=-1, select_idxs=[0]
    )
