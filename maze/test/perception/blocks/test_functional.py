...
def test_functional_block_single_arg():
    ...
    net: FunctionalBlock = FunctionalBlock(
        in_keys='in_key', out_keys='out_key', in_shapes=(100, 64, 1), func=torch.squeeze
    )
    str(net)
...
def test_functional_block_single_arg_lambda():
    ...
    in_dict = build_input_dict(dims=[100, 64, 1])
    net: FunctionalBlock = FunctionalBlock(
        in_keys='in_key', out_keys='out_key', in_shapes=(100, 64, 1), func=lambda value: torch.squeeze(value)
    )
    str(net)
    out_dict = net(in_dict)
...
def test_functional_block_single_arg_lambda_unsqueeze():
    ...
    net: FunctionalBlock = FunctionalBlock(
        in_keys='in_key', out_keys='out_key', in_shapes=(100, 64), func=lambda value: torch.unsqueeze(value, dim=-1)
    )
    str(net)
    out_dict = net(in_dict)
...
def test_functional_block_multi_arg():
    ...
    net: FunctionalBlock = FunctionalBlock(
        in_keys=['in_key_0', 'in_key_1'], out_keys='out_key', in_shapes=[(100, 64, 1), (100, 64, 1)], func=my_func
    )
    str(net)
    out_dict = net(in_dict)
...
def test_functional_block_multi_arg_order():
    ...
    net: FunctionalBlock = FunctionalBlock(
        in_keys=['in_key_0', 'in_key_1'], out_keys='out_key', in_shapes=[(100, 64), (100, 64, 1)], func=my_func
    )
    str(net)
    out_dict = net(in_dict)
