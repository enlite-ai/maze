...
from maze.core.agent.torch_state_critic import TorchDeltaStateCritic, TorchSharedStateCritic, TorchStepStateCritic
...
def test_custom_model_composer():
    ...
    value_0 = composer.critic.networks[0](
        convert_to_torch(env.observation_spaces_dict[0].sample(), device=None, cast=None, in_place=True)
    )
    _ = composer.critic.networks[1](
        {
            **convert_to_torch(env.observation_spaces_dict[1].sample(), device=None, cast=None, in_place=True),
...
def test_custom_model_composer_with_shared_embedding():
    ...
    policies = {
        ...
        'networks': [
            {
                '_target_': 'maze.perception.models.built_in.flatten_concat_shared_embedding.'
                'FlattenConcatSharedEmbeddingPolicyNet',
                'non_lin': 'torch.nn.SELU',
                'hidden_units': [16],
                'head_units': [16],
            },
    ...
    step_critic = {
        ...
        'networks': [
            {
                '_target_': 'maze.perception.models.built_in.flatten_concat_shared_embedding.'
                'FlattenConcatSharedEmbeddingStateValueNet',
                'non_lin': 'torch.nn.SELU',
                'head_units': [16],
            },
            {
