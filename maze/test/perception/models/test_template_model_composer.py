...
from maze.perception.models.critics import (
...
def build_single_step_with_critic_type(
    ...
    shared_embedding_keys: list[str] | dict[StepKeyType, list[str]] | None,
):
    """helper function"""
    # init environment
    ...
    modality_config['feature'] = {
        ...
        'block_params': {'hidden_units': [32, 32], 'non_lin': 'torch.nn.ReLU'},
    }
    modality_config['hidden'] = {
        'block_type': 'maze.perception.blocks.DenseBlock',
        'block_params': {'hidden_units': [64], 'non_lin': 'torch.nn.ReLU'},
    }
...
def test_default_models() -> None:
    ...
    build_single_step_with_critic_type(SharedStateCriticComposer, TorchSharedStateCritic, shared_embedding_keys=None)
    build_single_step_with_critic_type(StepStateCriticComposer, TorchStepStateCritic, shared_embedding_keys=None)
    build_single_step_with_critic_type(DeltaStateCriticComposer, TorchDeltaStateCritic, shared_embedding_keys=None)

    build_single_step_with_critic_type(StepStateCriticComposer, TorchStepStateCritic, shared_embedding_keys=['latent'])
    build_single_step_with_critic_type(
        StepStateCriticComposer, TorchStepStateCritic, shared_embedding_keys={0: ['observation_DenseBlock']}
    )
...
def test_default_models_multi_step() -> None:
    ...
    build_structured_with_critic_type(
        env, SharedStateCriticComposer, TorchSharedStateCritic, shared_embedding_keys=None
    )
...
def test_default_shared_model_multi_obs() -> None:
    ...
    build_structured_with_critic_type(
        env, StepStateCriticComposer, TorchStepStateCritic, shared_embedding_keys=['latent']
    )
    build_structured_with_critic_type(
        env,
        StepStateCriticComposer,
        TorchStepStateCritic,
        shared_embedding_keys=['observation_0_StridedConvolutionDenseBlock', 'observation_1_DenseBlock'],
    )
    with pytest.raises(AssertionError):
        build_structured_with_critic_type(
            env, StepStateCriticComposer, TorchStepStateCritic, shared_embedding_keys=['concat', 'observation_1']
        )
