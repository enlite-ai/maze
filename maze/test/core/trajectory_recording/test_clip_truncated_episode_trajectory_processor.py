"""Test the clip truncated episode trajectory processor."""
import copy

from maze.core.trajectory_recording.datasets.in_memory_dataset import InMemoryDataset
from maze.core.trajectory_recording.datasets.trajectory_processor import ClipTruncatedEpisodeTrajectoryProcessor
from maze.test.shared_test_utils.run_maze_utils import run_maze_job


def test_clip_truncated_true():
    ...
    terminated, truncated, info = ClipTruncatedEpisodeTrajectoryProcessor.retrieve_done_info(trajectory_record)
    ...
    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=2).pre_process(
        copy.deepcopy(trajectory_record))
    assert len(processed_trajectory) == len(trajectory_record) - 2

    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=5).pre_process(
        copy.deepcopy(trajectory_record))
    assert len(processed_trajectory) == len(trajectory_record) - 5

    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=0).pre_process(
        copy.deepcopy(trajectory_record))
    assert len(processed_trajectory) == len(trajectory_record) - 0

    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=1).pre_process(
        copy.deepcopy(trajectory_record))
    assert len(processed_trajectory) == len(trajectory_record) - 1

    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=20).pre_process(
        copy.deepcopy(trajectory_record))
    assert len(processed_trajectory) == 0


def test_clip_truncated_false():
    """Test loading trajectories of multiple episodes in parallel into an in-memory dataset. (Each
    data-loader process reads the files assigned to it.)"""
    # Heuristics rollout
    rollout_config = {
    ...
    assert terminated, f'info: {info}, {len(trajectory_record)}'
    assert not truncated

    processed_trajectory = ClipTruncatedEpisodeTrajectoryProcessor(clip_k=2).pre_process(trajectory_record)
