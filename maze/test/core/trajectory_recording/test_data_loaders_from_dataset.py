...
def test_sampling_from_flatten_in_memory_dataset():
    ...
    data_loader = get_maze_dataset_class(dataset).create_data_loader(
        dataset, batch_size=batch_size, num_workers=1, generator=rng
    )
    iteration = 0
    for iteration, data in enumerate(data_loader, 0):  # noqa: B007
        _, _, actor_ids = data[0], data[1], data[-1]
...
def test_sampling_from_split_dataset():
    ...
    data_loader = get_maze_dataset_class(train_set).create_data_loader(
        train_set, batch_size=batch_size, num_workers=1, generator=rng
    )
    n_batches_sampled = 0
