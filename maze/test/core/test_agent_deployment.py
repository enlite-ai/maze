...
from maze.core.log_stats.log_stats import LogStats, LogStatsLevel, LogStatsWriter, register_log_stats_writer
...
def test_steps_env_with_single_policy():
    agent_deployment = AgentDeployment(policy=DummyGreedyPolicy(), env=build_dummy_maze_env())
...
def test_supports_trajectory_recording_wrapper():
    ...
    agent_deployment = AgentDeployment(
        ...
        env=TrajectoryRecordingWrapper.wrap(build_dummy_maze_env(), serialize_renderer=True),
    )
...
def test_supports_multi_step_wrappers():
    ...
    agent_deployment.close(maze_state, reward, terminated, truncated, info)
    assert (
        env.get_stats_value(BaseEnvEvents.reward, LogStatsLevel.EPOCH, name='total_step_count') == 4
    )  # Step count is still 4 event with multiple sub-steps, as it is detected based on env time
...
def test_writes_event_and_stats_logs():
    ...
    agent_deployment = AgentDeployment(
        policy=DummyGreedyPolicy(), env=build_dummy_maze_env(), wrappers={LogStatsWrapper: {'logging_prefix': 'test'}}
    )
    ...
    for _ in range(step_count):
        maze_action = agent_deployment.act(
            maze_state, reward, terminated, truncated, info, events=list(test_core_env.get_step_events())
        )
...
def test_propagates_exceptions_to_main_thread():
    ...
    agent_deployment = AgentDeployment(policy=FailingPolicy(), env=build_dummy_maze_env())
...
def test_configures_from_hydra():
    ...
    agent_deployment = AgentDeployment(policy=cfg.policy, env=cfg.env, wrappers=cfg.wrappers)
    ...
    agent_deployment.close(maze_state, reward, terminated, truncated, info)
    assert (
        agent_deployment.env.get_stats_value(BaseEnvEvents.reward, LogStatsLevel.EPOCH, name='total_step_count') == 10
    )
