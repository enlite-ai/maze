...
def test_serialized_torch_policy():
    ...
    SpacesConfig(composer.action_spaces_dict, composer.observation_spaces_dict, composer.agent_counts_dict).save(
        'spaces_config.pkl'
    )

    # init policy
    policy = SerializedTorchPolicy(
        model=model_config, state_dict_file='state_dict.pt', spaces_dict_file='spaces_config.pkl', device='cpu'
    )
