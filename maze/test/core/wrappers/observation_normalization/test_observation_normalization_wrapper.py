...
from maze.core.log_stats.log_stats import increment_log_step, register_log_stats_writer
...
def random_env_steps(env: ObservationNormalizationWrapper, steps: int) -> np.ndarray:
    ...
    for _ in range(steps):
        action = env.sampling_policy.compute_action(
            obs, maze_state=None, env=env, actor_id=ActorID(0, 0), deterministic=False
        )
...
def test_observation_normalization_configs():
    ...
    normalization_config = {
        ...
        'statistics_dump': 'statistics.pkl',
        'exclude': None,
        'manual_config': {'observation': {'clip_range': (0, 1)}},
    }
...
def test_observation_normalization_init_from_yaml_config():
    ...
    norm_strategies = env._normalization_strategies
