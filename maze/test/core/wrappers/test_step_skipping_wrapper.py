...
def assertion_routine(env: StepSkipWrapper) -> None:
    ...
    for idx in range(n_substeps):
        if idx == 1:
            assert env._steps_done == 1  # noqa: SLF001

        if idx == 2:
            assert 0 in env._step_actions  # noqa: SLF001
            assert env._step_actions[env.actor_id()[0]] in env.action_space  # noqa: SLF001
    ...
    assert env._steps_done == 0  # noqa: SLF001
