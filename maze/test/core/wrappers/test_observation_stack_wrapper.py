...
def get_n_stacked_obs(stack_mode: str, env: ObservationStackWrapper, obs_key: str) -> int:
    """Return the number of stacked observations
    :param stack_mode: Specifies how stacking is done
    :param env: The environment
    :param obs_key: the observation key to check
    :return: The number of stacked observations labeled obs_key
    """
    if stack_mode == 'flatten_history':
        return len(env.get_observation_stack()[obs_key])
    if stack_mode == 'group_by_actor_id':
        return len(env.get_observation_stack()[env.actor_id()][obs_key])

    raise NotImplementedError(f'stack_mode: {stack_mode} not handled')
...
def test_observation_stack_wrapper(stack_mode):
    ...
    config = {
        'stack_config': [
            {'observation': 'observation_0', 'keep_original': False, 'tag': None, 'delta': True, 'stack_steps': 2},
            {'observation': 'observation_1', 'keep_original': True, 'tag': 'stacked', 'delta': False, 'stack_steps': 2},
        ]
    }
...
@pytest.mark.parametrize('stack_mode', ['flatten_history', 'group_by_actor_id'])
def test_observation_stack_init_from_yaml_config(stack_mode):
    ...
    # init environment
    env = build_dummy_structured_environment()
    env = ObservationStackWrapper.wrap(env, **config['observation_stack_wrapper'], stack_mode=stack_mode)
...
@pytest.mark.parametrize('stack_mode', ['flatten_history', 'group_by_actor_id'])
def test_observation_stack_wrapper_nothing_to_stack(stack_mode):
    """Observation stacking unit test"""
    ...
    config = {
        'stack_config': [
            {'observation': 'observation_0', 'keep_original': False, 'tag': None, 'delta': True, 'stack_steps': 1},
            {'observation': 'observation_1', 'keep_original': True, 'tag': 'stacked', 'delta': False, 'stack_steps': 1},
        ]
    }
...
@pytest.mark.parametrize('stack_mode', ['flatten_history', 'group_by_actor_id'])
def test_stack_reset_on_trajectory_load(stack_mode: str):
    env = build_dummy_maze_environment()

    # wrapper config
    stack_config = [
        {'observation': 'observation_0', 'keep_original': False, 'tag': 'stacked', 'delta': False, 'stack_steps': 2}
    ]
    env = ObservationStackWrapper.wrap(env, stack_config=stack_config, stack_mode=stack_mode)

    # == Check that stepping affects the stack as expected ==

    # Stack should be empty at the beginning
    assert get_n_stacked_obs(stack_mode, env, 'observation_0') == 0
    ...
    assert get_n_stacked_obs(stack_mode, env, 'observation_0') == 2
