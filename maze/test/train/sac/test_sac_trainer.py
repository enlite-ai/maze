...
class QCriticNetContinuous(nn.Module):
    ...
    def __init__(
        self,
        obs_shapes: dict[str, Sequence[int]],
        action_spaces_dict: dict[str | int, spaces.Space],  # noqa: ARG002
        non_lin: str | type(nn.Module),
    ):
        super().__init__()
        self.obs_shapes = obs_shapes
        ...
        self.perception_dict['latent-act'] = DenseBlock(
            in_keys='action', out_keys='latent-act', in_shapes=obs_shapes['action'], hidden_units=[256], non_lin=non_lin
        )

        self.perception_dict['concat'] = ConcatenationBlock(
            in_keys=['latent-obs', 'latent-act'],
            in_shapes=self.perception_dict['latent-obs'].out_shapes() + self.perception_dict['latent-act'].out_shapes(),
            concat_dim=-1,
            out_keys='concat',
        )
        ...
        self.perception_dict['q_value'] = LinearOutputBlock(
            in_keys='latent', out_keys='q_value', in_shapes=self.perception_dict['latent'].out_shapes(), output_units=1
        )
...
def train_function(
    ...
    env_factory = lambda: GymMazeEnv(env='LunarLanderContinuous-v3', render_mode=None)  # noqa: E731

    # initialize the env and enable statistics collection
    eval_env = distributed_env_cls([env_factory for _ in range(2)], logging_prefix='eval')
    ...
    action_shapes = {
        step_key: {
            action_head: tuple(distribution_mapper.required_logits_shape(action_head))
    ...
    # initialize policies
    policies = {
        ii: PolicyNet(obs_shapes=obs_shapes[ii], action_logits_shapes=action_shapes[ii], non_lin=nn.Tanh)
    ...
    # initialize critic
    critics = {
        ii: QCriticNetContinuous(obs_shapes[ii], non_lin=nn.Tanh, action_spaces_dict=env.action_spaces_dict)
        for ii in obs_shapes.keys()
    }
    ...
    algorithm_config = SACAlgorithmConfig(
        ...
        rollout_evaluator=RolloutEvaluator(
            eval_env=eval_env, n_episodes=eval_repeats, model_selection=None, deterministic=deterministic_eval
        ),
        patience=50,
    ...
    learner_model = TorchActorCritic(policy=actor_policy, critic=critics_policy, device='cpu')
...
def test_sac_trainer_keeping_rollouts():
    ...
    sac = train_function(
        ...
        eval_repeats=1,
        distributed_env_cls=SequentialVectorEnv,
