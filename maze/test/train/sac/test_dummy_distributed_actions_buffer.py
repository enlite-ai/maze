...
def create_dummy_distributed_actors(
    rollouts_per_iteration: int, n_rollout_steps: int, split_rollouts_into_transitions: bool, n_actors: int
):
    ...
    normalization_statistics = obtain_normalization_statistics(normalization_env, n_samples=10)
    ...
    SACRunner.init_replay_buffer(
        ...
        initial_sampling_policy=initial_sampling_policy,
    ...
    distributed_actors = DummyDistributedWorkersWithBuffer(
        env_factory=env_factory,
        worker_policy=model_composer.policy,
        n_rollout_steps=n_rollout_steps,
        n_workers=n_actors,
        batch_size=batch_size,
        rollouts_per_iteration=rollouts_per_iteration,
        split_rollouts_into_transitions=split_rollouts_into_transitions,
        env_instance_seeds=list(range(n_actors)),
        replay_buffer=replay_buffer,
    )
    ...
    if n_rollout_steps == 1 or not split_rollouts_into_transitions:
        assert len(distributed_actors.replay_buffer) == min(
            initial_buffer_size + rollouts_per_iteration, replay_buffer_size
        )
    elif split_rollouts_into_transitions:
        assert len(distributed_actors.replay_buffer) == min(
            initial_buffer_size + rollouts_per_iteration * n_rollout_steps, replay_buffer_size
        )
    ...
    for step_key in obs_shapes.keys():
        for obs_key, obs_shape in obs_shapes[step_key].items():
            if not split_rollouts_into_transitions:
                assert actor_output.observations[step_key][obs_key].shape == torch.Size(
                    [n_rollout_steps, batch_size, *obs_shape]
                )
            else:
                assert actor_output.observations[step_key][obs_key].shape == torch.Size([1, batch_size, *obs_shape])
...
def test_dummy_distributed_actors_single_rollout_step():
    """sac unit tests"""
    create_dummy_distributed_actors(
        rollouts_per_iteration=20, n_rollout_steps=1, split_rollouts_into_transitions=False, n_actors=1
    )
    create_dummy_distributed_actors(
        rollouts_per_iteration=20, n_rollout_steps=1, split_rollouts_into_transitions=True, n_actors=1
    )

    create_dummy_distributed_actors(
        rollouts_per_iteration=20, n_rollout_steps=1, split_rollouts_into_transitions=True, n_actors=2
    )
...
def test_dummy_distributed_actors_multiple_rollout_steps():
    """sac unit tests"""
    create_dummy_distributed_actors(
        rollouts_per_iteration=20, n_rollout_steps=3, split_rollouts_into_transitions=False, n_actors=1
    )
    create_dummy_distributed_actors(
        rollouts_per_iteration=20, n_rollout_steps=3, split_rollouts_into_transitions=True, n_actors=1
    )
