...
class DistributionMapper:
    ...
    def __init__(self, action_space: spaces.Dict, distribution_mapper_config: CollectionOfConfigType):
        ...
        for entry_dict in distribution_mapper_config:
            ...
            distribution_type = Factory(TorchProbabilityDistribution).type_from_name(entry_dict['distribution'])
    ...
    def action_head_distribution(
        self, action_head: str, logits: torch.Tensor, temperature: float
    ...
    def logits_dict_to_distribution(
        self, logits_dict: dict[str, torch.Tensor], temperature: float
        ...
        for action_head, action_logits in logits_dict.items():
            ...
            distribution_dict[action_head] = self.action_head_distribution(
                action_head=action_head, logits=action_logits, temperature=temperature
            )
