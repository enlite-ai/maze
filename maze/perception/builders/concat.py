...
class ConcatModelBuilder(BaseModelBuilder):
    ...
    def __init__(
        ...
        self.obs_to_block: dict[str, PerceptionBlock] = {}
        self.block_params = {}
    ...
    def from_observation_space(self, observation_space: spaces.Dict) -> InferenceBlock:
        ...
        for obs_key in observation_space.spaces.keys():
            if obs_key not in self.observation_modality_mapping:
                BColors.print_colored(
                    ...
                    f"model_builder.observation_modality_mapping and won't be considered as an input to the network.",
                    BColors.WARNING,
                )
                continue
            ...
            net = block_type(
                in_keys=obs_key, out_keys=f'{obs_key}_{block_type.__name__}', in_shapes=sample[obs_key].shape, **params
            )
        ...
        if (
            ConcatModelBuilderKeys.HIDDEN not in self.obs_to_block
