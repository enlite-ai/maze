...
class ProbabilisticPolicyComposer(BasePolicyComposer):
    ...
    def __init__(
        ...
        for substep_key in self._networks.keys():
            ...
            if substep_key in substeps_with_separate_agent_nets:
                assert agent_counts_dict[substep_key] != -1, (
                    'for separated agent policies, the agent count needs to be known upfront'
                )
    ...
    def _network_for_substep_key(self, key: StepKeyType):
        return Factory(nn.Module).instantiate(
            self._networks[key], obs_shapes=self._obs_shapes[key], action_logits_shapes=self._action_logit_shapes[key]
        )
