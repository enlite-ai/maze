...
class CustomModelComposer(BaseModelComposer):
    ...
    def check_model_config(cls, model_config: ConfigType) -> None:
        ...
        """
        if (
            'policy' in model_config
            and model_config['policy']
            and not isinstance(model_config['policy'], BasePolicyComposer)
        ):
            assert '_target_' in model_config['policy']
            assert 'networks' in model_config['policy'], (
                'Custom models expect explicit policy networks! Check the model config!'
            )
        if (
            'critic' in model_config
            and model_config['critic']
            and not isinstance(model_config['critic'], CriticComposerInterface)
        ):
            assert '_target_' in model_config['critic']
            assert 'networks' in model_config['critic'], (
                'Custom models expect explicit critic networks! Check the model config!'
            )
    ...
    def __init__(
        ...
        if critic is not None:
            critic_type = (
                Factory(CriticComposerInterface).type_from_name(critic['_target_'])
            ...
            if issubclass(critic_type, SharedStateCriticComposer):
                assert self.critic_input_spaces_dict == self.observation_spaces_dict, (
                    'Shared embedding is not yet supported for shared state critics'
                )
            ...
            if issubclass(critic_type, BaseStateCriticComposer):
                ...
                assert self.critic_input_spaces_dict == self.observation_spaces_dict, (
                    'Shared embedding is not yet supported for state-action critics'
                )
    ...
    def _build_critic_input_space_dict(self) -> dict[StepKeyType, spaces.Dict]:
        ...
        for step_key, obs_space in self.observation_spaces_dict.items():
            ...
            for obs_key, obs in obs_space.spaces.items():
                if (
                    isinstance(obs, spaces.Box)
                    and np.any(obs_space[obs_key].low == np.finfo(np.float32).min)
                    or np.any(obs_space[obs_key].high == np.finfo(np.float32).max)
                ):
                    ...
                    obs._np_random = None  # noqa: SLF001
