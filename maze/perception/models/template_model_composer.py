...
from maze.core.agent.torch_state_action_critic import (
    TorchSharedStateActionCritic,
    TorchStateActionCritic,
    TorchStepStateActionCritic,
)
from maze.core.agent.torch_state_critic import (
...
from maze.perception.models.critics import DeltaStateCriticComposer, SharedStateCriticComposer, StepStateCriticComposer
...
class TemplateModelComposer(BaseModelComposer):
    ...
    def check_model_config(cls, model_config: ConfigType) -> None:
        ...
        if 'policy' in model_config:
            ...
            assert 'networks' not in model_config['policy'], (
                'Template models do not expect explicit policy networks! Check the model config!'
            )
        if 'critic' in model_config and model_config['critic']:
            ...
            assert 'networks' not in model_config['critic'], (
                'Template models do not expect explicit critic networks! Check the model config!'
            )
    ...
    def __init__(
        ...
        model_builder: ConfigType | type[BaseModelBuilder],
        policy: ConfigType,
        critic: ConfigType,
    ):
        ...
        self._policy_type = (
            Factory(BasePolicyComposer).type_from_name(policy['_target_']) if policy is not None else None
        )
        self._critic_type = (
            Factory(CriticComposerInterface).type_from_name(critic['_target_']) if critic is not None else None
        )
        ...
        self._shared_embedding_nets: dict[StepKeyType, InferenceBlock] = {}
    ...
    def template_policy_net(
        self, observation_space: spaces.Dict, action_space: spaces.Dict, shared_embedding_keys: list[str]
        ...
        action_heads = []
        for action_head, _ in action_space.spaces.items():
        ...
        shared_embedding_keys_remove_input = list(
            filter(lambda x: x not in embedding_net.in_keys, shared_embedding_keys)
        )
        net = InferenceBlock(
            in_keys=embedding_net.in_keys,
            out_keys=action_heads + shared_embedding_keys_remove_input,
    ...
    def template_value_net(
        ...
        perception_net: InferenceBlock | None = None,
    ) -> InferenceBlock:
        ...
        if perception_net is None:
            ...
            _block_not_present_in_perception_net = list(
                filter(lambda x: x not in perception_net.perception_dict, shared_embedding_keys)
            )
            ...
            _blocks_with_more_outputs = list(
                filter(lambda x: len(perception_net.perception_dict[x].out_shapes()) > 1, shared_embedding_keys)
            )
            ...
            for _, block_keys in perception_net.execution_plan.items():
                ...
                if any([block_key in shared_embedding_keys for block_key in block_keys]):
                    assert all([[block_key in shared_embedding_keys for block_key in block_keys]]), (
                        'In the template model composer, all shared embedding keys given must be on the same level'
                        'in the execution plan of the inference block.'
                    )
                    start_adding_blocks = True

            inference_block_in_keys = shared_embedding_keys
            inference_block_in_shapes = sum(
                [perception_net.perception_dict[key].out_shapes() for key in shared_embedding_keys], []
            )
        ...
        # compile inference model
        net = InferenceBlock(
            in_keys=inference_block_in_keys,
            out_keys='value',
    ...
    def template_q_value_net(
        ...
        """
        assert all(map(lambda space: isinstance(space, (spaces.Discrete, spaces.Box)), action_space.spaces.values())), (
            'Only discrete and box spaces supported thus far for q values critic.'
        )

        if not only_discrete_spaces:
            discrete_space = list(
                filter(lambda kk: isinstance(action_space.spaces[kk], spaces.Discrete), action_space.spaces)
            )
        ...
        for action_head in action_space.spaces.keys():
            if action_head not in self.model_builder.observation_modality_mapping:
                BColors.print_colored(
                    ...
                    f"model_builder.observation_modality_mapping and won't be considered "
        ...
        # compile inference model
        net = InferenceBlock(
            in_keys=perception_net.in_keys,
    ...
    def critic(self) -> TorchStateCritic | TorchStateActionCritic | None:
        ...
        if self._critic_type is None:
            ...
            assert not any(self.model_builder.use_shared_embedding.values()), (
                'Embedding can not be shared when using a shared state critic'
            )
            ...
            return TorchSharedStateCritic(
                networks=critics, obs_spaces_dict=self.observation_spaces_dict, device='cpu', stack_observations=False
            )
            ...
            for sub_step_key, sub_step_space in self.observation_spaces_dict.items():
                critics[sub_step_key] = self.template_value_net(
                    observation_space=sub_step_space,
                    perception_net=self._shared_embedding_nets[sub_step_key],
                    shared_embedding_keys=self.model_builder.shared_embedding_keys[sub_step_key],
                )
            return TorchStepStateCritic(networks=critics, obs_spaces_dict=self.observation_spaces_dict, device='cpu')
            ...
            for idx, (sub_step_key, sub_step_space) in enumerate(self.observation_spaces_dict.items()):
                if idx > 0:
                    sub_step_space = spaces.Dict(
                        {**sub_step_space.spaces, **DeltaStateCriticComposer.prev_value_space.spaces}
                    )
                critics[sub_step_key] = self.template_value_net(
                    observation_space=sub_step_space,
                    perception_net=self._shared_embedding_nets[sub_step_key],
            ...
            return TorchDeltaStateCritic(networks=critics, obs_spaces_dict=self.observation_spaces_dict, device='cpu')
        elif issubclass(self._critic_type, SharedStateActionCriticComposer):
            assert not any(self.model_builder.use_shared_embedding.values()), (
                'Embedding can not be shared when using a shared state action critic'
            )
            ...
            critics = {
                0: self.template_q_value_net(observation_space, action_space, all(only_discrete_spaces.values()))
            }
            ...
            assert not any(self.model_builder.use_shared_embedding.values()), (
                'Embedding can not be shared when using a step state action critic'
            )
            critics = {}
            only_discrete_spaces = self._only_discrete_spaces()
            for sub_step_key, sub_step_space in self.observation_spaces_dict.items():
                critics[sub_step_key] = self.template_q_value_net(
                    observation_space=sub_step_space,
                    action_space=self.action_spaces_dict[sub_step_key],
                    only_discrete_spaces=only_discrete_spaces[sub_step_key],
                )
            return TorchStepStateActionCritic(
                networks=critics,
                num_policies=len(self.action_spaces_dict),
                device='cpu',
                only_discrete_spaces=only_discrete_spaces,
