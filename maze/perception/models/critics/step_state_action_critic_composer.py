...
class StepStateActionCriticComposer(BaseStateActionCriticComposer):
    ...
    def __init__(
        ...
        self._critics = {
            key: model_registry.instantiate(
                networks[key], obs_shapes=self._obs_shapes[key], output_shapes=critic_output_shapes[key]
            )
    ...
    def critic(self) -> TorchStepStateActionCritic:
        ...
        return TorchStepStateActionCritic(
            self._critics,
            num_policies=len(self._obs_shapes),
            device='cpu',
