...
class PerceptionBlock(ABC, nn.Module):
    ...
    def __init__(
        ...
        self.dummy_dict_creators: dict[int, Callable[[], torch.Tensor]] = {}
        for ii, _ in enumerate(self.in_keys):
    ...
    def out_shapes(self) -> list[Sequence[int]]:
        ...
        return [tuple(v.shape[1:]) if v.ndim > 1 else tuple(v.shape[0:]) for v in output_dict.values()]
