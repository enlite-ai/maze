...
class ActionMaskingBlock(PerceptionBlock):
    ...
    def __init__(
        ...
        in_shapes: list[Sequence[int]],
        num_actors: int,
        num_of_actor_actions: int | None,
    ):
        super().__init__(in_keys=in_keys, out_keys=out_keys, in_shapes=in_shapes)
        assert len(self.out_keys) == num_actors, 'The number of out_keys should be equal to the number of actors'
    ...
    def _forward_single(self, action_logits: torch.Tensor, action_mask: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        :param action_mask: the actions mask
        """
        assert action_logits.shape == action_mask.shape, (
            f'action_logtis.shape: {action_logits.shape} vs action_mask.shape: {action_mask.shape}'
        )
