...
class InferenceBlock(PerceptionBlock):
    ...
    def __init__(
        ...
        blocks_not_in_execution_graph = list(
            filter(lambda bk: bk not in sum(self.execution_plan.values(), []), self.perception_dict.keys())
        )
    ...
    def _test_condition(cls, perception_blocks: dict[str, PerceptionBlock], in_keys: list[str], out_keys: list[str]):
        ...
        assert len(set(all_out_keys)) == len(list(all_out_keys)), (
            f'{all_out_keys}, {len(set(all_out_keys))} vs {len(list(all_out_keys))}'
        )

        # 3. All block keys in the perception_blocks dict have to sub-strings of all their corresponding out_keys
        for block_key, block in perception_blocks.items():
            for out_key in block.out_keys:
                assert block_key in out_key, (
                    f'out_key ({out_key}) is not a superset of the block_key ({block_key}) it belongs to.'
                )
    ...
    def forward(self, block_input: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        tmp_dict: dict[str, torch.Tensor] = {}
    ...
    def _build_execution_graph(
        self, computed_keys: list[str], execution_plan: dict[int, list[str]]
    ) -> dict[int, list[str]]:
...
class InferenceGraph:
    ...
    def _draw(self, name: str, figure_size: tuple[int, int] | None) -> None:
        ...
        # draw edges
        for src_node, dst_node, _ in self.node_graph.edges(data=True):
    ...
    def _add_node_to_node_graph(self, block_key: str) -> tuple[nn.Module, str]:
        ...
        block_id = f'{curr_block.__class__.__name__}\n({block_key})'
        ...
        self.node_graph.add_node(
            block_id, label=label, is_block=True, num_params=num_of_block_params, block_id=block_key
        )
