...
class ImpalaRunner(TrainingRunner):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        if cfg.algorithm.rollout_evaluator.n_episodes > 0:
            if self.eval_concurrency > cfg.algorithm.rollout_evaluator.n_episodes:
                ...
                BColors.print_colored(
                    'IMPALARunner: number of parallel evaluation environments exceeds '
        ...
        train_agent_instance_seeds = [
            self.maze_seeding.generate_agent_instance_seed() for _ in range(cfg.algorithm.num_actors)
        ]
        rollout_actors = self.create_distributed_rollout_actors(
            self.env_factory,
            rollout_policy,
...
class ImpalaDevRunner(ImpalaRunner):
    ...
    def create_distributed_rollout_actors(
        ...
        n_actors: int,
        batch_size: int,
        queue_out_of_sync_factor: float,  # noqa: ARG002
        env_instance_seeds: list[int],
        agent_instance_seeds: list[int],  # noqa: ARG002
    ) -> SequentialDistributedActors:
        """Create dummy (sequentially-executed) actors."""
        return SequentialDistributedActors(
            env_factory, policy, n_rollout_steps, n_actors, batch_size, env_instance_seeds
        )

    def create_distributed_eval_env(
        self, env_factory: Callable[[], StructuredEnv | MazeEnv], eval_concurrency: int, logging_prefix: str
    ) -> SequentialVectorEnv:
...
class ImpalaLocalRunner(ImpalaRunner):
    ...
    def create_distributed_rollout_actors(
        ...
        return SubprocDistributedActors(
            env_factory,
            policy,
            n_rollout_steps,
            n_actors,
