...
class SACRunner(TrainingRunner):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        model = TorchActorCritic(
            policy=copy.deepcopy(self._model_composer.policy),
            critic=self._model_composer.critic,
            device=cfg.algorithm.device,
        )

        worker_env_instance_seeds = [
            self.maze_seeding.generate_env_instance_seed() for _ in range(cfg.algorithm.num_actors)
        ]
        ...
        if cfg.algorithm.rollout_evaluator.n_episodes > 0:
            if self.eval_concurrency > cfg.algorithm.rollout_evaluator.n_episodes:
                ...
                BColors.print_colored(
                    'SACRunner: number of parallel evaluation environments exceeds '
        ...
        if cfg.runner.initial_demonstration_trajectories:
            ...
            self.init_replay_buffer(
                ...
                replay_buffer_seed=replay_buffer_seed,
        ...
        distributed_actors = self.create_distributed_rollout_workers(
            ...
            n_rollout_steps=cfg.algorithm.n_rollout_steps,
            n_workers=cfg.algorithm.num_actors,
            batch_size=cfg.algorithm.batch_size,
            rollouts_per_iteration=cfg.algorithm.rollouts_per_iteration,
            split_rollouts_into_transitions=cfg.algorithm.split_rollouts_into_transitions,
            env_instance_seeds=worker_env_instance_seeds,
            replay_buffer=replay_buffer,
        )
    ...
    def load_replay_buffer(self, replay_buffer: BaseReplayBuffer, cfg: DictConfig) -> None:
        ...
        print(
            f'******* Starting to fill the replay buffer with trajectories from path: '
        ...
        with SwitchWorkingDirectoryToInput(cfg.input_dir):
            dataset = Factory(base_type=Dataset).instantiate(
                self.initial_demonstration_trajectories, conversion_env_factory=self.env_factory
            )
    ...
    def init_replay_buffer(
        ...
        initial_sampling_policy: DictConfig | Policy,
        ...
        sampling_policy: Policy = Factory(Policy).instantiate(
            initial_sampling_policy, action_spaces_dict=dummy_env.action_spaces_dict
        )
    ...
    def create_distributed_eval_env(
        self, env_factory: Callable[[], MazeEnv], eval_concurrency: int, logging_prefix: str
    ) -> StructuredVectorEnv:
...
class SACDevRunner(SACRunner):
    ...
    def create_distributed_rollout_workers(
        ...
        return DummyDistributedWorkersWithBuffer(
            ...
            env_instance_seeds=env_instance_seeds,
            replay_buffer=replay_buffer,
        )
