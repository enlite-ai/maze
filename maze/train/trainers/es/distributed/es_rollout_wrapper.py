...
class ESRolloutWorkerWrapper(Wrapper[StructuredEnv | LogStatsEnv]):
    ...
    def __init__(self, env: StructuredEnv | LogStatsEnv, shared_noise: SharedNoiseTable, agent_instance_seed: int):
    ...
    def rollout(self, policy: Policy | TorchModel) -> None:
        ...
        for _ in itertools.count():
            ...
            observation, reward, terminated, truncated, _ = self.step(
                convert_to_numpy(action, cast=None, in_place=False)
            )
    ...
    def generate_training(self, policy: Policy | TorchModel, noise_stddev: float) -> ESRolloutResult:
        ...
        v = torch.from_numpy(v).to(policy._device)  # noqa: SLF001
