...
class A2C(ActorCritic):
    ...
    def _update(self) -> None:
        ...
        # compute advantages
        advantages = [
            ret - detached_val for ret, detached_val in zip(returns, critic_output.detached_values, strict=False)
        ]
        ...
        if self.model.critic.num_critics == 1:
            ...
            value_losses = [(ret - val).pow(2).mean() for ret, val in zip(returns, critic_output.values, strict=False)]

        # compute policy loss, iterating across all sub-steps
        action_log_probs = policy_output.log_probs_for_actions(record.actions)
        policy_losses = []
        for advantage, action_log_probs_dict in zip(advantages, action_log_probs, strict=False):
