...
class MaskedStructuredCutting2DEnvironment(StructuredCutting2DEnvironment):
    ...
    def __init__(self, maze_env: MazeEnv):
        ...
        self._observation_spaces_dict[0].spaces['inventory_mask'] = gym.spaces.Box(
            low=np.float32(0), high=np.float32(1), shape=(max_inventory,), dtype=np.float32
        )

        self._observation_spaces_dict[1].spaces['cutting_mask'] = gym.spaces.Box(
            low=np.float32(0), high=np.float32(1), shape=(2,), dtype=np.float32
        )
    ...
    def _obs_cutting_step(flat_obs: dict[str, np.array], selected_piece_idx: int) -> dict[str, np.array]:
        ...
        return {'selected_piece': selected_piece, 'ordered_piece': ordered_piece, 'cutting_mask': cutting_mask}
