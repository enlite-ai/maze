...
class StructuredCutting2DEnvironment(Wrapper[MazeEnv], StructuredEnv, StructuredEnvSpacesMixin):
    ...
    def __init__(self, maze_env: MazeEnv):
        ...
        self._action_spaces_dict = {
            ...
            1: gym.spaces.Dict(
                {'cut_rotation': maze_env.action_space['cut_rotation'], 'cut_order': maze_env.action_space['cut_order']}
            ),
        }
        ...
        self._observation_spaces_dict = {
            0: flat_space,
            1: gym.spaces.Dict(
                {'selected_piece': flat_space['ordered_piece'], 'ordered_piece': flat_space['ordered_piece']}
            ),
        }
    ...
    def _obs_cutting_step(flat_obs: dict[str, np.array], selected_piece_idx: int) -> dict[str, np.array]:
        ...
        return {'selected_piece': flat_obs['inventory'][selected_piece_idx], 'ordered_piece': flat_obs['ordered_piece']}

    def _selection_step(self, action: dict[str, int]) -> tuple[dict[str, np.ndarray], float, bool, bool, dict]:
        """Cutting piece selection step."""
        self._action_0 = action
        obs = self._obs_cutting_step(self._flat_obs, action['piece_idx'])
        return obs, 0.0, False, False, {}

    def _cutting_step(self, action: dict[str, int]) -> tuple[dict[str, np.ndarray], float, bool, bool, dict]:
        """Cutting rotation and cutting order selection step."""
        flat_action = {
            'piece_idx': self._action_0['piece_idx'],
            'cut_rotation': action['cut_rotation'],
            'cut_order': action['cut_order'],
        }

        self._flat_obs, self._last_reward, terminated, truncated, info = self.env.step(flat_action)
        self._flat_obs['ordered_piece'] = self._flat_obs['ordered_piece']

        return self._obs_selection_step(self._flat_obs), self._last_reward, terminated, truncated, info
    ...
    def get_observation_and_action_dicts(
        self, maze_state: MazeStateType, maze_action: MazeActionType, first_step_in_episode: bool
        ...
        observation_dict, action_dict = self.env.get_observation_and_action_dicts(
            maze_state, maze_action, first_step_in_episode
        )
...
def struct_env_factory(
    max_pieces_in_inventory: int, raw_piece_size: tuple[int, int], static_demand: list[tuple[int, int]]
    ...
    env = maze_env_factory(
        max_pieces_in_inventory=max_pieces_in_inventory, raw_piece_size=raw_piece_size, static_demand=static_demand
    )
