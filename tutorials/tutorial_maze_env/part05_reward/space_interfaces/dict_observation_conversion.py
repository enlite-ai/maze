...
class ObservationConversion(ObservationConversionInterface):
    ...
    def maze_to_space(self, maze_state: Cutting2DMazeState) -> dict[str, np.ndarray]:
        ...
        return {
            'inventory': np.asarray(inventory_state, dtype=np.float32),
    ...
    def space(self) -> spaces.Dict:
        ...
        :return: Gym space object
            ...
            - order: 2d vector holding x/y-dimension of customer ordered piece
        """
        return spaces.Dict(
            {
                'inventory': spaces.Box(
                    low=np.zeros((self.max_pieces_in_inventory, 2), dtype=np.float32),
                    high=np.vstack(
                        [[self.raw_piece_size[0] + 1, self.raw_piece_size[1] + 1]] * self.max_pieces_in_inventory
                    ).astype(np.float32),
                    dtype=np.float32,
                ),
                'inventory_size': spaces.Box(
                    low=np.float32(0), high=self.max_pieces_in_inventory + 1, shape=(1,), dtype=np.float32
                ),
                'ordered_piece': spaces.Box(
                    low=np.float32(0), high=np.float32(max(self.raw_piece_size) + 1), shape=(2,), dtype=np.float32
                ),
            }
        )
