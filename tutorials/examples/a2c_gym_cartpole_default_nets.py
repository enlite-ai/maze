...
def main(n_epochs: int, rnn_steps: int) -> None:
    ...
    envs = SequentialVectorEnv(
        [lambda: to_rnn_dict_space_environment(env=env_name, rnn_steps=rnn_steps) for _ in range(4)],
    ...
    modality_config['feature'] = {
        ...
        'block_params': {'hidden_units': [32, 32], 'non_lin': 'torch.nn.Tanh'},
    }
    modality_config['hidden'] = {
        'block_type': 'maze.perception.blocks.DenseBlock',
        'block_params': {'hidden_units': [64], 'non_lin': 'torch.nn.Tanh'},
    }
    modality_config['recurrence'] = {}
    if rnn_steps > 0:
        modality_config['recurrence'] = {
            'block_type': 'maze.perception.blocks.LSTMLastStepBlock',
            'block_params': {'hidden_size': 8, 'num_layers': 1, 'bidirectional': False, 'non_lin': 'torch.nn.Tanh'},
        }
