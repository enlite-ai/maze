...
def main(n_epochs: int, rnn_steps: int) -> None:
    ...
    envs = DummyStructuredDistributedEnv([lambda: to_rnn_dict_space_environment(env=env_name, rnn_steps=rnn_steps)
                                          for _ in range(4)],
                                         logging_prefix="train")

    # initialize the env and enable statistics collection
    eval_env = DummyStructuredDistributedEnv([lambda: to_rnn_dict_space_environment(env=env_name, rnn_steps=rnn_steps)
                                              for _ in range(4)],
                                             logging_prefix="eval")
    ...
    template_builder = TemplateModelComposer(
        ...
        policy={'type': 'maze.perception.models.policies.ProbabilisticPolicyComposer'},
        critic={'type': 'maze.perception.models.critics.StateCriticComposer'})
