...
.. code-block:: bash
    ...
    maze-run -cn conf_rollout env.name=CartPole-v1 policy=random_policy runner=sequential runner.render=true
...
.. code-block:: bash
    ...
    maze-run -cn conf_train env.name=CartPole-v1 algorithm=a2c model=vector_obs critic=template_state input_dir=outputs/<experiment-dir>

Run a rollout of a policy, trained with the command above:

.. code-block:: bash

    maze-run -cn conf_rollout env.name=CartPole-v1 model=vector_obs policy=torch_policy input_dir=outputs/<experiment-dir>