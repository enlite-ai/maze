...
class PolicyNet(nn.Module):
    ...
    def __init__(self, obs_shapes: dict[str, Sequence[int]], action_logits_shapes: dict[str, Sequence[int]]):
        ...
        self.net = nn.Sequential(
            nn.Linear(in_features=obs_shapes[OBSERVATION_NAME][0], out_features=16),
            nn.Tanh(),
...
distribution_mapper = DistributionMapper(
    action_space=spaces.Dict(spaces={ACTION_NAME: spaces.Discrete(2)}), distribution_mapper_config={}
)
...
policy_net = PolicyNet(obs_shapes={OBSERVATION_NAME: (4,)}, action_logits_shapes={ACTION_NAME: logits_shape})
