...
def build_perception_dict():
    ...
    in_dict = build_multi_input_dict(dims=[[1, 16], [1, 3, 64, 64]], names=obs_keys)

    perception_dict = {}

    # --- block ---
    net = DenseBlock(
        in_keys='obs_inventory',
        out_keys='obs_inventory_latent',
    ...
    # --- block ---
    net = VGGConvolutionDenseBlock(
        in_keys='obs_screen',
        out_keys='obs_screen_latent',
    ...
    # --- block ---
    net = ConcatenationBlock(
        in_keys=list(perception_dict.keys()), out_keys='concat', in_shapes=[(32,), (32,)], concat_dim=-1
    )
    perception_dict['concat'] = net

    # --- block ---
    net = LinearOutputBlock(in_keys=['concat'], out_keys='action_move', in_shapes=[(64,)], output_units=4)
    perception_dict['action_move'] = net

    # --- block ---
    net = LinearOutputBlock(in_keys=['concat'], out_keys='action_use', in_shapes=[(64,)], output_units=16)
...
if __name__ == '__main__':
    ...
    net = InferenceBlock(
        in_keys=['obs_inventory', 'obs_screen'],
