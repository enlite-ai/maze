...
from docs.source.policy_and_value_networks.code_snippets.custom_cartpole_critic_net import CustomCartpoleCriticNet
from docs.source.policy_and_value_networks.code_snippets.custom_cartpole_policy_net import CustomCartpolePolicyNet
...
def test_cartpole_policy_model():
    ...
    policy = CustomCartpolePolicyNet(
        obs_shapes[0], action_logits_shapes[0], non_lin='torch.nn.ReLU', hidden_units=[16, 32]
    )

    critic = CustomCartpoleCriticNet(obs_shapes[0], non_lin='torch.nn.ReLU', hidden_units=[16, 32])
...
def test_cartpole_model_composer():
    env = GymMazeEnv(env='CartPole-v1', render_mode=None)
    path_to_model_config = code_snippets.__path__._path[0] + '/custom_cartpole_net.yaml'  # noqa: SLF001
