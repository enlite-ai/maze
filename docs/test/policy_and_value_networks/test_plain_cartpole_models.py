...
def test_cartpole_policy_model():
    ...
    policy = CustomPlainCartpolePolicyNet(
        obs_shapes[0], action_logits_shapes[0], hidden_layer_0=16, hidden_layer_1=32, use_bias=True
    )
...
def test_cartpole_model_composer():
    env = GymMazeEnv(env='CartPole-v1', render_mode=None)
    path_to_model_config = code_snippets.__path__._path[0] + '/custom_plain_cartpole_net.yaml'  # noqa: SLF001
